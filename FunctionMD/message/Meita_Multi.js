...
// const ordermessage = (m.xtype === 'conversation' && m.message.conversation) ? m.message.conversation : (m.xtype == 'imageMessage') && m.message.imageMessage.caption ? m.message.imageMessage.caption : (m.xtype == 'documentMessage') && m.message.documentMessage.caption ? m.message.documentMessage.caption : (m.xtype == 'videoMessage') && m.message.videoMessage.caption ? m.message.videoMessage.caption : (m.xtype == 'extendedTextMessage') && m.message.extendedTextMessage.text ? m.message.extendedTextMessage.text : (m.xtype == 'buttonsResponseMessage' && m.message.buttonsResponseMessage.selectedButtonId) ? m.message.buttonsResponseMessage.selectedButtonId : (m.xtype == 'templateButtonReplyMessage') && m.message.templateButtonReplyMessage.selectedId ? m.message.templateButtonReplyMessage.selectedId : ''
   ...
   const botNumber = meita.user.id.split(':')[0] + '@s.whatsapp.net'
   ...
   const groupMetdata = isGroup ? await meita.groupMetadata(from) : ''
   const groupName = isGroup ? await groupMetdata.subject : ''   
   const isAntiLink = isGroup ? antilink.includes(m.sender) : false
   const groupMetadata = isGroup ? await meita.groupMetadata(from) : ''
...
const sleep = async (ms) => {
  return new Promise(resolve => setTimeout(resolve, ms));
   ...
   meita.sendMessage(from, 
        { text: teks, mentions: [m.sender] },
        { quoted : m })  
    }      
   
// Auto Read & Presence Online
meita.sendReadReceipt(from, m.sender, [m.key.id])
meita.sendPresenceUpdate('available', from)
let fkontak = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `Meita-Multi`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;rifza;;;\nFN:rifza\nitem1.TEL;waid=6287708357324:6287708357324\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}               

meita.setStatus = (status) => {
        meita.query({
            tag: 'iq',
            attrs: {
                to: '@s.whatsapp.net',
...
await meita.setStatus(`ACTIVE DURING : ${runtime(process.uptime())} TYPE ${prefix}menu`) 
	}*/
   ...
   const ikan = ['🐟','🐠','🐡']     
 // Game
cekWaktuGame(meita, tebakgambar)
...
if (chatmessage.includes('https://chat.whatsapp.com')) {
               ...
               reply('Antilink\nKamu akan di kick')
             let number = m.sender
               await meita.groupParticipantsUpdate(from, [number], 'remove')
               }
	  }		
...
await sleep(45000) 
await meita.updateBlockStatus(from, "block")*/
...
console.log(result)
meita.sendMessage(from, { image : { url : result.thumbnail}, caption : `*- Username:* ${result.username}\n*- Fullname:* ${result.fullname}\n*- Followers:* ${result.followers}\n*- Following:* ${result.following}\n*- Verified:* ${result.verified}\n*- Bio:* ${result.bio}`}, {quoted : m}) 
}) 
break
...
let emo1 = arags[0]
let emo2 = arags[1]
meita.sendMessage(from, { sticker : { url : `https://tenor.googleapis.com/v2/featured?key=AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCYQ&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v5&q=${encodeURIComponent(emo1)}_${encodeURIComponent(emo2)}`}}) 
}
...
meita.sendMessage(from, { image : { url : anime.img }, caption : rem}, { quoted : m }) 
break
...
_Mengirim file..._`
reply(result4)
meita.sendMessage(from, { document : { url : baby1[0].link}, fileName : baby1[0].nama, mimetype: baby1[0].mime }, { quoted : m }) 
break
case 'aesthetic':
...
meita.sendMessage(from, { image : { url : Diep }, caption : 'nih' }, { quoted : m}) 
break
case 'storywa':
if (q.includes('--help')) return reply(examkosong) 
if (isLimit < 1) return reply("Limit kamu sudah habis ಥ╭╮ಥ, silahkan beli dengan cara #buy limit _jumlah_")
...
meita.sendMessage(from, { video : { url : Diep1.url }, caption : 'nih' }) 
break
...
reply(`Satu limit terpakai ಥ‿ಥ\nSisa limit kamu : ${getLimit(m.sender)}`) 
  ...
  var buffer = Buffer.from([])
   for await(const chunk of stream) {
    ...
    reply('success')
    meita.sendMessage(from, { image: { url: './' + filename + '.jpg' }}, { quoted: m })
  ...
  var buffer = Buffer.from([])
   ...
   let filename = Math.ceil(Math.random() * 99999)
    ...
    reply('success')
    meita.sendMessage(from, { video: { url: './' + filename + '.mp4' }}, { quoted: m })
   ...
   for await(const chunk of stream) {
     ...
     meita.sendMessage(from, { video: { url: data.result }}, { quoted: m })
     })
 ...
 try{
  if (!m.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated !== true){
    ...
    var buffer = Buffer.from([])
     ...
     meita.sendMessage(from, { video: { url : data.result }, caption: "Nih", gifPlayback: true}, { quoted: m } )
     })
    } 
...
reply(`Satu limit terpakai ಥ‿ಥ\nSisa limit kamu : ${getLimit(m.sender)}`) 
   ...
   meita.sendMessage(from, { audio: {url : './777.mp3'}, mimetype: 'audio/mp4'}, {quoted: m})    
   }
  break
 case 'tovn':{
if (isLimit < 1) return reply("Limit kamu sudah habis ಥ╭╮ಥ, silahkan beli dengan cara #buy limit _jumlah_")
...
reply(`Satu limit terpakai ಥ‿ಥ\nSisa limit kamu : ${getLimit(m.sender)}`) 
 try{
   if (m.message.extendedTextMessage.contextInfo.quotedMessage.audioMessage){
    ...
    var buffer = Buffer.from([])
     ...
     fs.writeFileSync(`./101010.mp3`, buffer)
      meita.sendMessage(from, { audio: {url : './101010.mp3'}, mimetype: 'audio/mp4', ptt: true}, {quoted: m})
   ...
   fs.writeFileSync('./' + 777 + '.mp3', buffer)
   meita.sendMessage(from, { audio: {url : './777.mp3'}, mimetype: 'audio/mp4', ptt: true}, {quoted: m})    
   } 
   ...
   if (!args[0]) return reply('Linknya mana?')
    ...
    let anu = await igDownloader(`${q}`)
     ...
     if (linck.includes("mp4")){
       meita.sendMessage(from, { video: { url : linck }, caption: desksz}, { quoted: m } )
      } else 
     if (linck.includes("jpg")){
       meita.sendMessage(from, { image: { url : linck }, caption: desksz}, { quoted: m } )
     }
     console.log(data)
    }
    )
...
const sami = simi.success
reply(sami) 
//meita.sendMessage(from, { text : sami },m) 
break
case 'lirik':
if (q.includes('--help')) return reply(examquery) 
...
meita.sendMessage(from, {text : `Kasus : ${kasus}\n\nKematian : ${kematian}\n\nSembuh : ${sembuh}`}, m)
break
//=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×=×
case 'gempa':
if (q.includes('--help')) return reply(examkosong) 
...
meita.sendMessage(from, { image : { url : Map }, caption : captt})
break
case 'chara':
if (q.includes('--help')) return reply(examquery) 
...
await meita.sendMessage(from, {image : { url : acak }, caption : 'Nih bang'})
break
break
...
meita.sendMessage(from, { image: { url: data2.image }, caption: teks }, { quoted: m })
.then( res => {
...
meita.sendMessage(from, { image : { url : 'https://telegra.ph/file/75ffe3024a0ff8d563da5.jpg' }, caption : teks }) 
})
}
break
...
await meita.groupUpdateDescription(from, q)
break
...
if (q == "close") {
  meita.groupSettingUpdate(from, 'announcement')
  reply(`*Sukses menutup group*`)
} else if (q == "open") {
  meita.groupSettingUpdate(from, 'not_announcement')
...
if (!isGroupAdmins) return reply('Khusus Admin')
if (!isBotGroupAdmins) return reply('Bot Bukan Admins')
await meita.groupRevokeInvite(from)
break
case 'setnamegroup':
if (q.includes('--help')) return reply(examkosong) 
if (!isGroup) return reply('Khusus Grup')
...
await meita.groupUpdateSubject(from, q)
break
case 'wiki':
if (q.includes('--help')) return reply(examquery) 
...
meita.sendMessage(from, { image : { url : res2[0].thumb }, caption : result2}) 
break
case 'asupan':
if (q.includes('--help')) return reply(examquery) 
...
console.log(xm2.url) 
meita.sendMessage(from, {video : { url : xm2.url }, caption : 'nih'}) 
...
console.log(x2.url) 
meita.sendMessage(from, {video : { url : x2.url }, caption : 'nih'}) 
...
reply(`Satu limit terpakai ಥ‿ಥ\nSisa limit kamu : ${getLimit(m.sender)}`) 
    ...
    var buffer = Buffer.from([])
     ...
     meita.sendMessage(from, { image: { url: `./666.jpg` }}, { quoted: m })
   }
  break
 case 'tomp4':{
if (q.includes('--help')) return reply(examplyme) 
...
meita.sendMessage(from, {image : { url : server[10].thumb }, caption : cn}) 
break
case 'manga':{
if (q.includes('--help')) return reply(examquery) 
...
*Synopsis:* ${synopsis}
*Link*: ${url}`
meita.sendMessage(from, {image : { url : image_url }, caption: mangaingfo})
} 
  break
case 'broadcast':
if (q.includes('--help')) return reply(examquery) 
...
for (let i of bc) {
meita.sendMessage(i.id, { text: `*[ DEFFBOTZ BROADCAST ]*\n\n${q}` })
}
break
case 'couple': {
if (q.includes('--help')) return reply(examkosong) 
...
meita.sendMessage(from, { image: { url: random.male }, caption: `Couple Male` }, { quoted: m })
meita.sendMessage(from, { image: { url: random.female }, caption: `Couple Female` }, { quoted: m })
            }
	    break
 case 'speed':
if (q.includes('--help')) return reply(examkosong) 
...
if (q.includes('--help')) return reply(examquery) 
    if (args.length < 1) return m.reply(from, 'Teks?', { quoted : m } )
     ...
     console.log(anu)
    meita.sendMessage(from, { image: { url : anu }, caption: `Nih kak, jangan lupa follow ig owner\n${Options.info.igowner}`}, { quoted: m } )
...
meita.sendMessage(from, {image : { url : 'https://i.ibb.co/Kw282gw/b54b1faadf3b.jpg' }, caption : result}) 
break	
case 'hidetag':
if (q.includes('--help')) return reply(examquery) 
...
meita.sendMessage(from, { text: q ? q : '', mentions: mem })
break
case 'pinterest': {
if (q.includes('--help')) return reply(examquery) 
reply('Tunggu Sebentar')
...
meita.sendMessage(from, { image: { url: result }, caption: '▹ Media Url : '+result }, { quoted: m })
            }
            break
  case 'antilink':
...
meita.sendMessage(from,  {text: `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`})
} else if (q === 'off') {
...
if (q.includes('--help')) return reply(examkosong) 
  ...
  let buttons = [
    ...
    let buttonMessage = {
     ...
     meita.sendMessage(from, buttonMessage, { quoted: m })
...
if (q.includes('--help')) return reply(examkosong) 
 ...
 case 'owner': case 'creator': {
   ...
   meita.sendContact(from, x.split('@s.whatsapp.net')[0], Options.info.ownerName, m)
	}			    
   }
  break
  case 'artinama':{
...
const anj = await brainly(q)
  ...
  meita.sendMessage(from, {image : { url : 'https://telegra.ph/file/ba004c72b2222f4913e99.jpg' }, caption : hmm2}) 
...
.then((a) => {
      ...
      meita.sendMessage(from, { audio: { url: dl_link }, mimetype: 'audio/mp4' }, { quoted: m })
      })
...
.then((a) => {
      ...
      meita.sendMessage(from, { video: { url: dl_link }, caption: "This is the result\nHope you are happy with our service😊" }, { quoted: m })
      })
...
if (q.includes('--help')) return reply(examplyme) 
  ...
  console.log(mentionUser)
		await meita.groupParticipantsUpdate(
		 from, 
		 mentionUser, 
		 "promote"
		 )
...
if (q.includes('--help')) return reply(examply) 
      if (!m.quoted) return  m.reply(from, 'Reply pesanya!', { quoted : m })
       if (!m.quoted.isBaileys) return  m.reply(from, 'Fitur ini hanya berlaku menghapus pesan bot yang di kirim oleh saya!', { quoted : m })
          meita.sendMessage(from, { delete: { remoteJid: from, fromMe: true, id: m.quoted.id, participant: m.quoted.sender } })
         }
      break
  case 'wall':
...
if (q.includes('--help')) return reply(examlink) 
 ...
 reply(pesan.eror) 
   ...
   const musim_duren_a = musim_rambutan.result.nowatermark
    meita.sendMessage(from, { audio: { url: musim_duren_a }, mimetype: 'audio/mp4' }, { quoted: m })
   }
 break
...
var igs = arags[0]
var igs2 = arags[1]
var a = await axios.get(`https://megayaa.herokuapp.com/api/igstori?username=${args[0]}`)
var l = await fetchJson(`https://megayaa.herokuapp.com/api/igstori?username=${args[0]}`)
 
 var result1 = `${a.data.data[args[1] - 1].url}`
 await meita.sendMessage(from, { video: { url: result1 }, caption: `*Succses*\nInstagram story dari https://instagram.com/${l.username}/` }, { quoted: m })
...
case 'getfoto':
  ...
  kurangLimit(m.sender, 1)
  reply(`Satu limit terpakai ಥ‿ಥ\nSisa limit kamu : ${getLimit(m.sender)}`)
if (!args.length) return reply(from, "Urutan keberapa? #getimage/getvideo urutan", lin)
var a = await axios.get(`https://megayaa.herokuapp.com/api/igstori?username=${args[0]}`)
var mm = await fetchJson(`https://megayaa.herokuapp.com/api/igstori?username=${args[0]}`)

 var result1 = `${a.data.data[args[1] - 1].url}`
 await meita.sendMessage(from, { image: { url: result1 }, caption: `*Succses*\nInstagram story dari https://instagram.com/${mm.username}/` }, { quoted: m })

break
case 'igstory':
var a = await axios.get(`https://megayaa.herokuapp.com/api/igstori?username=${q}`)
...
for (let i = 0; i < a.data.data.length; i++) {
 ...
 const list = {title: `${a.data.username}`,
rows: [
    {
     title: `Pencarian ${a.data.username} `, 
     rowId: `#get${a.data.data[i].type === "mp4" ? "video" : "foto"} ${a.data.username} ${i + 1}`,
     description: `💻 Type : ${a.data.data[i].type === "mp4" ? "video 🎥" : "foto 📸"} ${i + 1}`
    }, 
    ]
   }
   sections.push(list)   
...
const sendm =  meita.sendMessage(
    from, 
    {
     text: `Berikut IGSTORY ${a.data.username}`,
     footer: Options.info.botName,
     title: "[ IGSTORY SEARCH 🔎 ]",
     buttonText: "Click and see search results➡️",
     sections
    }, { quoted : m }
  )  
...
igstalk(q).then(result => {
console.log(result)
let buttons = [{ buttonId: `${prefix}igstory ${result.username}`, buttonText: { displayText: `IGStory ${result.username}` }, type: 1}]
    let buttonMessage = {
      image: { url: result.thumbnail },
      caption: `*- Username:* ${result.username}\n*- Fullname:* ${result.fullname}\n*- Followers:* ${result.followers}\n*- Following:* ${result.following}\n*- Verified:* ${result.verified}\n*- Bio:* ${result.bio}`,
      footer: `Tekan Dibawah Untuk Mendownload Story ${result.username}\n\n${Options.info.botName}`,
      buttons: buttons,
      headerType: 4
     }
     meita.sendMessage(from, buttonMessage, { quoted: m })
...
await meita.groupAcceptInvite(resultt)
.then((res) => reply(jsonformat(`Success`)))
.catch((err) => reply(jsonformat(`Maaf terjadi Kesalahan`)))
}
break
case 'add': {
  if (!isGroup) return reply('Khusus Grup')
 ...
 if (!isBotGroupAdmins) return reply('Bot Bukan Admin')
  ...
  await meita.groupParticipantsUpdate(from, [users], 'add')
  .then((res) => reply(jsonformat(`Success`)))
  .catch((err) => reply(jsonformat(`Maaf terjadi Kesalahan`)))
}
           break
 ...
 if (!isGroupAdmins) return reply('Khusus Admin')
 if (!isBotGroupAdmins) return reply('Bot Bukan Admin')
            let users = mentionUser[0] ? mentionUser[0] : m.quoted ? m.quoted.sender : q.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
            await meita.groupParticipantsUpdate(from, [users], 'remove')
            .then((res) => reply(jsonformat(`Success`)))
            .catch((err) => reply(jsonformat(`Maaf terjadi Kesalahan`)))
          }
          break
          ...
          case 'vote': {
            if (!isGroup) return reply('Khusus Grup')
            if (from in vote) throw reply(`_Masih ada vote di chat ini!_\n\n*${prefix}hapusvote* - untuk menghapus vote`)
            if (!q) return reply(`Masukkan Alasan Melakukan Vote, Example: *${prefix + order} Owner Ganteng*`)
            vote[from] = [q, [], []]
...
meita.sendMessage(from, teks_vote1, m)
        }, 5000)}
            break
               case 'upvote': {
                if (!isGroup) return reply('Khusus Grup')
            if (!(from in vote)) throw reply(`_*tidak ada voting digrup ini!*_\n\n*${prefix}vote* - untuk memulai vote`)
          var  isVote = vote[from][1].concat(vote[from][2])
...
*Alasan:* ${vote[from][0]} dari ${vote[from][1].map((v) => `@${v.split`@`[0]}`).join('\n')}
...
*${prefix}hapusvote* - untuk menghapus vote`
            ...
            meita.sendMessage(from, buttonMessageDevote)
	}
            break
...
*${prefix}hapusvote* - untuk menghapus vote
`
meita.sendMessage(from, teks_vote, m)
break
      ...
      if (!isGroup) return reply('Khusus Grup')
            ...
            break
            case 'timer':
              reply(`Memulai Hitungan Mundur`)
              if (args[1]=="detik") {var timer = args[0]+"000"
              } else if (args[1]=="menit") {var timer = args[0]+"0000"
              } else if (args[1]=="jam") {var timer = args[0]+"00000"
              } else {return reply("*pilih:*\ndetik\nmenit\njam contoh .timer 10 detik")}
              setTimeout( () => {
              reply("Waktu habis")
              }, timer)
              break
